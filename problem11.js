...
let x_dim = data[0].length;
let y_dim = data.length;

let curr_greatest_prod = 0;

for(let i = 0; i < x_dim; i++){
    for(let j = 0; j < y_dim; j++){
        let horz_prod = 0;
        if(j < y_dim - 3){
            horz_prod = data[i][j] * data[i][j+1] * data[i][j+2] * data[i][j+3]; 
        ...
        let vert_prod = 0;
        if(i < x_dim - 3){
            vert_prod = data[i][j] * data[i+1][j] * data[i+2][j] * data[i+3][j];
...
console.log(curr_greatest_prod);